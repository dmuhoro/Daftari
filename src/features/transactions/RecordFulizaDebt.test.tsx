...
import RecordFulizaDebt from './RecordFulizaDebt'

vi.mock('../../lib/store', () => {
  const storeState = {
    language: 'sw',
    addTransaction: vi.fn(async () => 'fd-1'),
  }
...
describe('RecordFulizaDebt', () => {
  ...
  it('renders amount input and buttons', () => {
    render(<RecordFulizaDebt onSave={onSave} onCancel={onCancel} />)
    expect(screen.getByPlaceholderText('0')).toBeDefined()
    expect(screen.getByText('save')).toBeDefined()
    expect(screen.getByText('cancel')).toBeDefined()
  })

  it('shows fuliza label', () => {
    render(<RecordFulizaDebt onSave={onSave} onCancel={onCancel} />)
    expect(screen.getByText('chukua_fuliza')).toBeDefined()
  })
