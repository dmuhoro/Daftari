...
import { toKES, kesSum, kesSubtract } from '../lib/money';
...
export default function PosScreen({ onBack }: PosScreenProps) {
  ...
  const cartTotal = useMemo(() => kesSum(cart.map(i => toKES(i.price * i.qty))), [cart]);
  const maxRedeem = selectedCustomer ? Math.floor((selectedCustomer.loyalty_points ?? 0) / POINTS_REDEEM_RATE) : 0;
  const discount = toKES(redeemPoints ? Math.min(maxRedeem, cartTotal) : 0);
  const finalTotal = kesSubtract(cartTotal, discount);
  ...
  async function handleCheckout() {
    ...
    setReceiptData({
      ...
      type: 'income',
      description: cart.map(i => `${i.name} x${i.qty}`).join(', '),
      items: cart.map(i => ({ name: i.name, qty: i.qty, price: toKES(i.price * i.qty) })),
            ...
            <div className="max-h-32 overflow-y-auto flex flex-col gap-1 mb-2">
              {cart.map((item) => (
                <div key={item.productId} className="flex items-center gap-2 text-sm">
                  ...
                  <span className="w-20 text-right text-xs text-primary-600 font-semibold">KES {toKES(item.price * item.qty).toLocaleString('en-KE')}</span>
                </div>
              ))}
            </div>
